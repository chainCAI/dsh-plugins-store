...
const CATEGORY_PATTERN_RULES: TopicPatternRule<Category>[] = [
  ...
  {
    pattern: /messag|notif|chat|(^|-)bots?($|-)|wechat|wecom|weixin|qq-bot|telegram|feishu|lark|email|imap|cross-session-messaging|interconnect/,
  ...
  {
    pattern: /mcp|model-(provider|routing|pool)|llm-(provider|routing|gateway)|ai-sdk-provider|openai-compatible/,
    weights: { 'model-mcp': 4 },
  },
  {
    pattern: /security|safety|audit|approval|permission|sandbox|guard|secret|policy|credential|redact|privacy|sast|scanning|workspace-safety/,
    weights: { security: 4 },
  },
  {
    pattern: /deploy|docker|kubernetes|helm|(^|-)ops($|-)|monitor|observability|telemetry|diagnostic|doctor|health-check|runtime-health|performance|cost|token-(usage|cost|budget|tracking)|billing|quota|scheduler|cron|backup|migration|restart|self-heal|tracing/,
