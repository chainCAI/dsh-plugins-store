...
- **收录统计**：首页实时展示已收录插件数、精选数、分类数与数据更新时间
- **分类引擎**：基于仓库 Topics 的加权规则分类（项目类型 + 功能分类），按使用场景筛选
- **精选机制**：Star 数 ≥ 100 即标记为"精选"（蓝色徽章），规则简单透明
- **站长精选**：站长为每个分类按 Star 精选前 3 个插件并附推荐理由（编辑内容维护于 `src/data/curated.ts`）；头部导航与首页筛选 tab 均可进入独立页面，Star 实时取自目录
- **状态徽章**：精选 / 新收录（7 天内）/ 已归档，颜色语义克制明确
- **搜索筛选**：支持名称、作者、描述、标签搜索；按类型、分类筛选；按 Star / 更新时间 / 名称排序
- **本地收藏**：点击书签收藏插件，首页可筛选"我的收藏"，数据保存在浏览器 localStorage
- **标签跳转**：卡片标签可点击，跳转到对应标签聚合页
- **自动更新**：GitHub Actions 每 30 分钟同步一次，catalog.json 有变化即提交，push 自动触发 Cloudflare Pages 重建
...
仓库内置 `.github/workflows/sync-catalog.yml`：每 30 分钟运行一次同步，`src/data/catalog.json` 有变化时自动提交并推送，从而触发 Cloudflare Pages 自动重建。启用后无需手动操作。
...
- 只收录 DeepSeek Harness 公测（2026-08-13）之后创建的仓库，公测前创建的蹭标签项目不收录；例外是「站长精选」清单（`src/data/curated.ts`）中的仓库，它们经白名单由同步脚本单独补充收录，不受公测时间过滤；
